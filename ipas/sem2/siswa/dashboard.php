...
if ($level_kelas == 0 || $id_guru_siswa == 0) {
    $stmt_user = $db_mapel->prepare("SELECT kelas, id_guru FROM " . tbl('users') . " WHERE id = ? AND role = 'siswa'");
...
if ($id_guru_siswa > 0) {
    $stmt_g = $conn->prepare("SELECT nama_lengkap FROM " . tbl('users') . " WHERE id = ? AND role = 'guru'");
...
if ($id_guru_siswa > 0) {
    ...
    $list_nilai_kuis = [];
    $q_kuis_per_mapel = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) as nilai_bulat_materi FROM " . tbl('riwayat_kuis') . " WHERE id_user = ? GROUP BY id_materi");
    ...
    $list_nilai_to = [];
    $q_to_per_judul = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) as nilai_bulat_to FROM " . tbl('riwayat_tryout') . " WHERE id_user = ? GROUP BY tryout_id");
    ...
    $q_stat = $db_mapel->prepare("SELECT 
        ...
        FROM " . tbl('riwayat_kuis') . " WHERE id_user = ?");
    ...
    $q_to_count = $db_mapel->prepare("SELECT COUNT(DISTINCT tryout_id) as total_to FROM " . tbl('riwayat_tryout') . " WHERE id_user = ?");
...
<head>
    ...
    <title>Dashboard Siswa | IPAS</title>
    ...
    <style>
        ...
        .hero-card {
            background: <?= $is_archive ? 'linear-gradient(135deg, #6c757d, #495057)' : 'linear-gradient(135deg, #198754, #146c43)' ?>;
        ...
        .guide-banner-ipas { background: #fff; border: none; border-radius: 15px; border-left: 6px solid #198754; }
...
<nav class="navbar navbar-dark <?= $is_archive ? 'bg-secondary' : 'bg-primary' ?> shadow-sm">
    <div class="container">
        <div>
            <a class="navbar-brand fw-bold" href="#">IPAS | SISWA</a>
        </div>
...
<div class="container mt-5 mb-5">
    <div class="hero-card text-center mb-5">
        <h1 class="display-5 fw-bold"><i class="fas fa-leaf"></i> Peta Pembelajaran Anda</h1>
        <p class="lead mt-3">Selamat datang kembali, <?php echo htmlspecialchars($nama_pengguna); ?>! Mari selesaikan materi IPAS hari ini bersama <b>Guru <?php echo htmlspecialchars($nama_guru_pembimbing); ?></b>.</p>
        <p class="fs-4">Anda berada di <b>Kelas <?php echo htmlspecialchars($level_kelas); ?></b>.</p>
        <a href="../../../dashboard.php" class="btn btn-light mt-2 fw-bold text-success border-2">
            <i class="fas fa-th"></i> Pilih Mata Pelajaran
        </a>
    </div>

    <div class="row mb-5">
        <div class="col-12">
            <div class="card guide-banner-ipas shadow-sm">
                <div class="card-body p-4 d-flex align-items-center justify-content-between flex-wrap">
                    <div>
                        <h5 class="fw-bold mb-1 text-success"><i class="fas fa-magic me-2"></i> Belum Tahu Cara Memulainya?</h5>
                        <p class="text-muted mb-0 small">Klik tombol di samping untuk panduan cara belajar, mengerjakan kuis, dan bantuan AI.</p>
                    </div>
                    <button class="btn btn-success fw-bold px-4 rounded-pill mt-3 mt-md-0 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalPanduanIpas">
    ...
    <div class="row text-center mb-5">
        ...
        <div class="col-md-4 mb-3">
            <div class="stat-card">
                <h5 class="card-title text-primary">Nilai Rata-Rata</h5>
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            <div class="stat-card p-4">
                ...
                <p>Pilih materi IPAS yang ingin Anda taklukkan hari ini.</p>
                <a href="materi.php" class="btn btn-primary w-100 btn-lg">
                    <i class="fas fa-map-marked-alt"></i> Lihat Materi
                </a>
            </div>
        </div>
        <div class="col-md-6 mb-3">
        ...
        <div class="col-md-6 mb-3">
            <div class="stat-card p-4">
                <h4><i class="fas fa-rocket"></i> Mulai Try Out</h4>
                <p>Pilih kategori ujian: Harian (Materi), Tengah Semester, atau Semester.</p>
                <a href="daftar_tryout.php" class="btn btn-primary w-100 btn-lg mt-2">
                    <i class="fas fa-list"></i> Pilih Kategori Ujian
                </a>
            </div>
        </div>
        <div class="col-md-6 mb-3">
        ...
        <!--        <a href="../penilaian_materi.php" class="btn btn-warning w-100 btn-lg mt-2 text-white">-->
...
<div class="modal fade" id="modalPanduanIpas" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; border: none;">
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-success"><i class="fas fa-star me-2"></i> Cara Belajar IPAS</h5>
            ...
            <div class="modal-body p-4 text-start">
                <div class="d-flex mb-4">
                    ...
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Pilih Materi</h6>
                        <p class="small text-muted mb-0">Klik tombol <b>Lihat Materi</b> dan pilih bab yang ingin dipelajari.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    ...
                    <div class="ms-3">
                        ...
                        <p class="small text-muted mb-0">Jika bingung dengan istilah sains yang sulit, tanya <b>Kak PADI AI</b> di pojok kanan bawah.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    <div class="bg-success text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 45px; height: 45px; flex-shrink: 0; font-weight: bold;">3</div>
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Cek Hasil & Ujian</h6>
                        <p class="small text-muted mb-0">Gunakan menu <b>Riwayat</b> untuk melihat skor dan ikuti <b>Tryout</b> untuk menguji kemampuanmu.</p>
                    </div>
                </div>
            </div>
            <div class="modal-footer border-0 p-4 pt-0">
                <button type="button" class="btn btn-success w-100 rounded-pill py-2 fw-bold" data-bs-dismiss="modal">SIAP BELAJAR!</button>
            </div>
        </div>
    </div>
</div>
...
<div id="ai-chat-container">
    <div id="ai-chat-window">
        <div class="chat-header">
            <span><i class="fas fa-robot me-2"></i> Asisten PADI IPAS</span>
            <button class="btn-close btn-close-white" onclick="toggleChat()"></button>
        </div>
        <div class="chat-body" id="chatBody">
            <div class="msg msg-ai">Halo <strong><?= htmlspecialchars($nama_pengguna) ?></strong>! Ada yang bisa Bapak bantu soal fenomena alam atau materi IPAS hari ini? 😊</div>
            <div id="typingIndicator" class="typing">AI sedang berpikir...</div>
        </div>
        <div class="chat-footer">
            <input type="text" id="chatInput" class="form-control form-control-sm" placeholder="Tanya tentang IPAS..." onkeypress="handleKeyPress(event)">
