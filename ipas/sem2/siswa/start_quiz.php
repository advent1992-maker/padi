<?php
// FILE: siswa/start_quiz.php - SESUAI LOGIKA GAMBAR MATEMATIKA
...
// --- LOGIKA PATH GAMBAR (DISAMAKAN DENGAN MTK) ---
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($soal_ids as $soal_id) {
        ...
        $skor += $skor_soal;
        $detail_data[] = ['soal_id' => $soal_id, 'jawaban_siswa' => $jawaban_sis, 'skor_soal' => $skor_soal];
    }
...
// === Tampilkan Soal (MENGAMBIL KOLOM GAMBAR OPSI) ===
$query_soal = "SELECT id, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, 
               ...
               gambar_url FROM soal WHERE materi_id = ? ORDER BY RAND() LIMIT 50";
...
<head>
    ...
    <title><?= htmlspecialchars($page_title); ?> | IPAS</title>
...
<div class="container mt-5">
    <h1 class="text-primary">KUIS IPAS</h1>
    ...
    <form method="POST" action="start_quiz.php?materi_id=<?= $id_materi; ?>">
        <?php foreach ($soal_list as $index => $soal): ?>
            <div class="quiz-card">
                <h5>Soal <?= $index + 1; ?>.</h5>
                <div class="fs-5 mb-2"><?= $soal['pertanyaan']; ?></div>
                ...
                <?php foreach (['a','b','c','d'] as $opt): ?>
                    <div class="form-check mt-3">
                        ...
                        <label class="form-check-label" for="soal_<?= $soal['id'].'_'.$opt; ?>">
