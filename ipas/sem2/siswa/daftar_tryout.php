...
// Query untuk mengambil daftar Try Out dengan RATA-RATA SKOR
$query = "
    SELECT 
        tm.id, 
        tm.judul, 
        ...
        -- Ambil total riwayat count
        (SELECT COUNT(rt.id) FROM riwayat_tryout rt WHERE rt.tryout_id = tm.id AND rt.id_user = ?) AS riwayat_count,

        -- SEKARANG: Ambil RATA-RATA skor dari semua pengerjaan
        (
            SELECT AVG(rt.skor) 
            FROM riwayat_tryout rt 
            WHERE rt.tryout_id = tm.id AND rt.id_user = ?
        ) AS avg_skor,

        -- SEKARANG: Ambil RATA-RATA persentase dari semua pengerjaan
        (
            SELECT AVG(rt.persentase) 
            FROM riwayat_tryout rt 
            WHERE rt.tryout_id = tm.id AND rt.id_user = ?
        ) AS avg_persentase
...
<head>
    ...
    <title>Daftar Try Out | MATHFICTION</title>
...
.score-number {
    font-size: 2.2rem; /* Ukuran angka diperbesar */
    font-weight: 900;
    line-height: 1;
    color: #0d6efd; /* Warna biru biar kontras */
}

/* Jika Lulus, warna angka jadi Hijau */
.border-success .score-number {
    color: #198754;
}

/* Jika Gagal (seperti di gambar Anda), warna angka bisa disesuaikan */
.border-danger .score-number {
    color: #dc3545;
}
    </style>
</head>
    ...
    <div class="row">
        ...
        <?php else: ?>
            ...
            <?php
                ...
                // Ambil nilai rata-rata (dibulatkan agar rapi)
                    ...
                    <div class="card-body">
                        ...
                        <?php if ($is_done): ?>
                            <div class="p-2 mt-2 mb-3 border rounded border-success bg-light">
                                <p class="mb-1 small fw-bold text-success">
                                    <i class="fas fa-check-circle me-1"></i> Sudah Dikerjakan (<?php echo $riwayat_count; ?>x)
                                </p>
                                <hr class="my-1">
                                <p class="mb-1 small">
