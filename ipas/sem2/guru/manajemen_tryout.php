...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adopsi_tryout'])) {
    ...
    header("Location: manajemen_tryout.php?semester=" . $semester);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_tryout'])) {
    ...
    $stmt = $db_mapel->prepare("DELETE FROM " . tbl('soal_tryout') . " WHERE tryout_id=?");
$stmt->bind_param("i",$tryout_id);
$stmt->execute();
$stmt->close();

$stmt = $db_mapel->prepare("DELETE FROM " . tbl('riwayat_tryout') . " WHERE tryout_id=?");
$stmt->bind_param("i",$tryout_id);
$stmt->execute();
$stmt->close();
...
$query_pribadi = "SELECT tm.*, COUNT(st.id) AS total_soal FROM " . tbl('tryout_master') . " tm
LEFT JOIN " . tbl('soal_tryout') . " st ON tm.id = st.tryout_id WHERE tm.kelas IN ($placeholders) AND tm.id_guru = ? GROUP BY tm.id ORDER BY tm.id DESC";
...
LEFT JOIN " . tbl('soal_tryout') . " st
                  ON tm.id = st.tryout_id 
...
<?php if(!$isArsip): ?>
    ...
    <a href="form_soal_tryout.php?tryout_id=<?= $tryout['id']; ?>"
       class="btn btn-sm btn-success mb-1">
       <i class="fas fa-pencil-alt"></i> Kelola Soal
    </a>

    <a href="ai_generator_tryout_page.php?tryout_id=<?= $tryout['id']; ?>"
       class="btn btn-sm btn-info text-white mb-1">
       <i class="fas fa-robot"></i> Buat dengan AI
    </a>

    <a href="ambil_kuis_proses.php?tryout_id=<?= $tryout['id']; ?>"
       class="btn btn-sm btn-info text-white mb-1">
    ...
    <button type="button"
        ...
        data-id="<?= $tryout['id']; ?>"
        data-judul="<?= htmlspecialchars($tryout['judul']); ?>"
        data-kelas="<?= htmlspecialchars($tryout['kelas']); ?>"
        data-waktu="<?= htmlspecialchars($tryout['waktu_alokasi']); ?>"
        data-jenis="<?= htmlspecialchars($tryout['jenis_ujian']); ?>">
...
<?php endif; ?>
    ...
    <a href="preview_tryout_cetak.php?tryout_id=<?= $tryout['id'] ?>"
       class="btn btn-sm btn-info text-white fw-bold mb-1">
