...
$is_arsip = ($_SESSION['semester_aktif'] ?? '2') == '1';
$pesan = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        $db_mapel->query("DELETE FROM " . tbl('riwayat') . " WHERE id_materi = $delete_id");
...
$query_adopsi = "SELECT m.*, COUNT(s.id) AS jumlah_soal FROM materi m
                 LEFT JOIN " . tbl('soal') . " s ON m.id = s.materi_id
...
<head>
    <meta charset="UTF-8">
    <title>Kelola Materi | IPAS</title>
...
<body>
    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <?php if (!$is_arsip): ?>
<a href="materi_form.php" class="btn btn-primary">
        ...
        <div class="card card-custom p-4 mb-5">
            ...
            <?php else: ?>
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>Judul</th><th>Kelas</th><th>Kuis</th><th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
...
<a href="kuis_form.php?id_materi=<?= $m['id'] ?>" class="btn btn-sm btn-outline-primary">
    Kelola Kuis
</a>

<a href="materi_form.php?id=<?= $m['id'] ?>" class="btn btn-sm btn-warning">
    <i class="fas fa-edit"></i>
</a>

<button class="btn btn-sm btn-danger"
        onclick="confirmDelete(<?= $m['id'] ?>,'<?= $m['judul'] ?>')">
    <i class="fas fa-trash"></i>
</button>

<?php endif; ?>

<a href="view_ifp.php?id=<?= $m['id'] ?>"
   target="_blank"
   class="btn btn-success btn-sm rounded-pill">
    ...
    <a href="materi_adopsi_proses.php?id_materi=<?= $a['id'] ?>"
       ...
       onclick="return confirm('Adopsi materi ini menjadi milik Anda? Anda akan bisa mengedit dan melihat progresnya sebagai materi pribadi.')">
        <i class="fas fa-copy"></i> Adopsi Materi
    </a>
</td>
                        </tr>
                        <?php endforeach; ?>
