...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && ($action === 'add' || $action === 'edit')) {
    ...
    if ($action === 'add') {
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->execute()) {
                $_SESSION['admin_message'] = "<div class='alert alert-success'>✅ Materi <b>$judul</b> berhasil ditambahkan!</div>";
            } else {
                $_SESSION['admin_message'] = "<div class='alert alert-danger'>❌ Gagal menambah materi. Error: " . $stmt->error . "</div>";
            }
            $stmt->close();
        }
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->execute()) {
                $_SESSION['admin_message'] = "<div class='alert alert-success'>✅ Materi <b>$judul</b> berhasil diperbarui!</div>";
            } else {
                $_SESSION['admin_message'] = "<div class='alert alert-danger'>❌ Gagal memperbarui materi. Error: " . $stmt->error . "</div>";
            }
            $stmt->close();
        }
    }
...
if ($action === 'delete' && $id > 0) {
    ...
    try {
        // 1. Hapus Soal yang terkait
        $stmt_soal = $conn->prepare("DELETE FROM soal WHERE id_materi = ?");
        $stmt_soal->bind_param("i", $id);
        $stmt_soal->execute();
        $stmt_soal->close();

        // 2. Hapus Materi
        $stmt_materi = $conn->prepare("DELETE FROM materi WHERE id = ?");
        $stmt_materi->bind_param("i", $id);
        $stmt_materi->execute();
        $stmt_materi->close();

        $conn->commit();
        $_SESSION['admin_message'] = "<div class='alert alert-success'>✅ Materi ID $id dan soal terkait berhasil dihapus!</div>";
    } catch (Exception $e) {
        $conn->rollback();
        $_SESSION['admin_message'] = "<div class='alert alert-danger'>❌ Gagal menghapus materi.</div>";
    }
    ...
    if ($stmt_fetch = $conn->prepare($query_fetch)) {
        ...
        if ($result_fetch->num_rows === 1) {
            $materi_data = $result_fetch->fetch_assoc();
             ...
             $_SESSION['admin_message'] = "<div class='alert alert-danger'>Materi tidak ditemukan.</div>";
             header("Location: manage_materi.php");
             exit();
        }
        $stmt_fetch->close();
...
<head>
    ...
    <title>Kelola Materi Mathfiction | Admin</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>

<div class="container mt-5">
    <h1>Kelola Materi Cerita 📚 (Akses Admin)</h1>
    ...
    <?php if ($action === 'add' || $action === 'edit'): ?>
        ...
        <div class="card p-4 mb-4">
            ...
            <form method="POST" action="manage_materi.php?action=<?php echo $action; ?><?php echo ($action === 'edit' ? '&id=' . $id : ''); ?>">
                ...
                <div class="mb-4">
                    ...
                    <small class="form-text text-muted">Arahkan ke file HTML/PHP yang berisi narasi visual yang menarik.</small>
                </div>
    ...
    ?>
        ...
        <p><a href="manage_materi.php?action=add" class="btn btn-success">➕ Tambah Bab Cerita Baru</a></p>
                ...
                <tbody>
                    <?php if ($result_read && $result_read->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td><small><?php echo htmlspecialchars($materi['file_path']); ?></small></td>
                            <td>
                                <a href="manage_materi.php?action=edit&id=<?php echo $materi['id']; ?>" class="btn btn-warning btn-sm me-2">Edit</a>
                                <a href="manage_materi.php?action=delete&id=<?php echo $materi['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('YAKIN hapus materi dan semua soal terkait?')">Hapus</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
