...
// Ambil string kelas yang diajar dari sesi (misal: '1,5,6')
$kelas_diajar_str = $_SESSION['kelas'] ?? '';
$kelas_in_clause = "NULL"; // Default aman

if (!empty($kelas_diajar_str) && $id_guru_login > 0) {
    // Sanitize dan buat klausa IN
    $kelas_array = explode(',', $kelas_diajar_str);
    // Kita pastikan setiap elemen adalah digit (kelas)
...
// 3. Peringkat Dihitung menggunakan PHP (usort)
            ...
            <tbody>
                ...
                <?php if (!empty($leaderboard_data)): ?>
                    ...
                    <?php foreach ($leaderboard_data as $index => $row): ?>
                        <?php
                            // Logika penentuan peringkat sama skor = sama peringkat
                        ...
                        <tr class="<?php echo $row_class; ?>">
                            <td>
                                <?php
                                // Logika tampilan ikon peringkat
                                if ($peringkat == 1) {
                                    echo '<i class="fas fa-trophy text-warning me-2"></i>';
                                } elseif ($peringkat == 2) {
                                    echo '<i class="fas fa-medal text-secondary me-2"></i>';
                                } elseif ($peringkat == 3) {
                                    echo '<i class="fas fa-medal me-2" style="color:#8b4513;"></i>';
                                }
                                echo $peringkat;
                                ?>
                            </td>
                            ...
                            <td class="text-end fw-bold"><?php echo htmlspecialchars($row['final_score'] ?? '0'); ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
