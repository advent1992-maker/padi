...
<head>
    ...
    <title><?= $materi ? htmlspecialchars($materi['judul']) : 'Materi'; ?> | PJOK</title>
    ...
    <style>
        ...
        .result-card { border: 1px solid #dee2e6; border-left: 5px solid #fd7e14; padding: 15px; border-radius: 8px; background-color: #fffaf5; }
...
<nav class="navbar navbar-expand-lg navbar-dark shadow-sm" style="background-color: #fd7e14;">
    <div class="container">
        <a class="navbar-brand fw-bold" href="dashboard.php"><i class="fas fa-running me-2"></i>PJOK</a>
        <a href="../logout.php" class="btn btn-sm btn-light fw-bold" style="color: #fd7e14;">Keluar</a>
    </div>
</nav>
        ...
        <button onclick="toggleFullScreen()" class="btn btn-dark btn-sm rounded-pill px-3 shadow-sm">
            ...
            <?php if ($has_quiz): ?>
                ...
                <?php if ($materi['tampilkan_kuis'] == 1): ?>
                    <div class="alert alert-warning text-center py-4 <?= ($jumlah_percobaan >= 5) ? 'limit-reached' : ''; ?>" style="background-color: #fff9f0; border: 1px solid #ffeeba;">
                        <h6 class="fw-bold">Uji Kemampuan PJOK</h6>
                        <p class="mb-2">Percobaan: <b><?= $jumlah_percobaan; ?> / 5</b></p>
                        
                        <?php if ($jumlah_percobaan < 5): ?>
                            <p class="small">Sudah siap ujian praktis? Ayo kerjakan kuisnya!</p>
                            <a href="start_quiz.php?materi_id=<?php echo $materi['id']; ?>" class="btn btn-warning px-5 fw-bold shadow" style="background-color: #fd7e14; border-color: #fd7e14; color: white;">
                        ...
                        <?php else: ?>
                            <div class="alert alert-danger d-inline-block rounded-pill px-4 mt-2">
                                <i class="fas fa-exclamation-triangle me-2"></i> Batas pengerjaan (5x) sudah tercapai.
                            </div>
                        <?php endif; ?>
                    </div>
                <?php else: ?>
                    <div class="alert alert-secondary text-center py-3 rounded-pill shadow-sm">
