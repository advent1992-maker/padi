...
// Sinkronisasi data user (Gunakan $conn untuk tabel users portal)
if ($level_kelas == 0 || $id_guru_siswa == 0) {
    $stmt_user = $conn->prepare("SELECT kelas, id_guru FROM users WHERE id = ? AND role = 'siswa'");
...
// Inisialisasi default (Logika Statistik Tetap Pakai $db_mapel)
$total_bab_lulus = 0;
$total_bab_dicoba = 0;
...
<head>
    ...
    <title>Dashboard Siswa | B.INDO</title>
    ...
    .btn-primary { background-color: #e65100 !important; border-color: #e65100 !important; }
    .text-primary { color: #e65100 !important; }
    .hero-card {
        background: <?= $is_archive ? 'linear-gradient(135deg, #6c757d, #495057)' : 'linear-gradient(135deg, #a52a2a, #e65100)' ?>;
    ...
    .text-biru-pilihan { color: #0d6efd !important; }
    
    /* Style Banner Panduan */
    .guide-banner-indo { background: #fff; border: none; border-radius: 15px; border-left: 6px solid #e65100; }

    /* STYLE AI CHAT DENGAN LABEL DI BAWAH */
    #ai-chat-container {
...
<nav class="navbar navbar-dark <?= $is_archive ? 'bg-secondary' : 'bg-primary' ?> shadow-sm">
    <div class="container">
        <a class="navbar-brand fw-bold" href="#">B.INDO | SISWA</a>
        <span class="navbar-text text-white">
            Halo, <b class="text-white"><?php echo htmlspecialchars($nama_pengguna); ?></b> (Kelas <?php echo htmlspecialchars($level_kelas); ?>)
        </span>
...
<div class="container mt-5 mb-5 text-start">
    <div class="hero-card text-center mb-4">
        <h1 class="display-5 fw-bold"><i class="fas fa-book"></i> Peta Pembelajaran Bahasa Indonesia</h1>
        <p class="lead mt-3">Selamat belajar kembali bersama <b>Guru <?php echo htmlspecialchars($nama_guru_pembimbing); ?></b>.</p>
        <a href="../../../dashboard.php" class="btn btn-light mt-2 fw-bold text-biru-pilihan shadow-sm rounded-pill">
            <i class="fas fa-th-large me-2"></i> Pilih Mata Pelajaran
        </a>
    </div>

    <div class="row mb-5">
        <div class="col-12">
            <div class="card guide-banner-indo shadow-sm">
                <div class="card-body p-4 d-flex align-items-center justify-content-between flex-wrap">
                    <div>
                        <h5 class="fw-bold mb-1 text-primary"><i class="fas fa-info-circle me-2"></i> Petunjuk Belajar Bahasa Indonesia</h5>
                        <p class="text-muted mb-0 small">Pelajari alur materi, kuis, dan penggunaan Asisten AI di sini.</p>
                    </div>
                    <button class="btn btn-primary fw-bold px-4 rounded-pill mt-3 mt-md-0 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalPanduanIndo">
                        <i class="fas fa-map me-2"></i> LIHAT PANDUAN
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="row text-center mb-5">
        <div class="col-md-4 mb-3">
            <div class="stat-card h-100">
                <h5 class="card-title text-success small fw-bold">MATERI SELESAI</h5>
                <h1 class="text-success my-3 fw-bold"><?php echo $lulus_tampil; ?></h1>
            </div>
        </div>
        <div class="col-md-4 mb-3">
            <div class="stat-card h-100">
                <h5 class="card-title text-primary small fw-bold">TOTAL TUGAS</h5>
                <h1 class="text-primary my-3 fw-bold"><?php echo $total_bab_dicoba; ?></h1>
            </div>
        </div>
        <div class="col-md-4 mb-3">
            <div class="stat-card h-100">
                <h5 class="card-title text-success small fw-bold">NILAI RATA-RATA</h5>
    ...
    <div class="row g-4">
        <div class="col-md-6">
            <div class="stat-card p-4 h-100">
                <h4>Daftar Materi dan Kuis</h4>
                <p class="text-muted small">Pilih bab Bahasa Indonesia yang ingin dipelajari.</p>
                <a href="materi.php" class="btn btn-primary w-100 btn-lg rounded-pill">
                    <i class="fas fa-list-ul me-2"></i> Buka Materi
                </a>
            </div>
        </div>
        <div class="col-md-6">
            <div class="stat-card p-4 h-100">
                <h4>Riwayat Belajar dan Pembahasan</h4>
                <p class="text-muted small">Lihat skor kuis dan Pembahasan.</p>
                <a href="riwayat_progress.php" class="btn btn-success w-100 btn-lg rounded-pill border-0" style="background-color: #2ecc71;">
        ...
        <div class="col-md-4">
            <div class="stat-card p-4">
                <h5 class="fw-bold"><i class="fas fa-pen-nib text-primary"></i> Try Out</h5>
                <a href="daftar_tryout.php" class="btn btn-outline-primary w-100 mt-2 rounded-pill small">Mulai Ujian</a>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card p-4">
                <h5 class="fw-bold"><i class="fas fa-medal text-warning"></i> Peringkat</h5>
                <a href="leaderboard.php" class="btn btn-outline-warning w-100 mt-2 rounded-pill small text-dark">Leaderboard</a>
            </div>
        </div>
        <!--<div class="col-md-4">-->
        <!--    <div class="stat-card p-4">-->
        <!--        <h5 class="fw-bold"><i class="fas fa-edit text-dark"></i> Ulasan</h5>-->
        <!--        <a href="../penilaian_materi.php" class="btn btn-outline-dark w-100 mt-2 rounded-pill small">Isi Penilaian</a>-->
        <!--    </div>-->
        <!--</div>-->
    </div>
</div>

<div class="modal fade" id="modalPanduanIndo" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; border: none;">
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-primary"><i class="fas fa-book-reader me-2"></i> Alur Belajar Bahasa Indonesia</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 text-start">
                <div class="d-flex mb-4">
                    <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 40px; height: 45px; flex-shrink: 0; font-weight: bold;">1</div>
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Membaca Materi</h6>
                        <p class="small text-muted mb-0">Klik <b>Buka Materi</b> dan pilih bab. Baca teks bacaan dengan saksama.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    <div class="bg-info text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 40px; height: 45px; flex-shrink: 0; font-weight: bold; background-color: #764ba2 !important;">2</div>
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Gunakan Asisten AI</h6>
                        <p class="small text-muted mb-0">Jika bingung dengan kata sulit, tanya <b>Kak PADI AI</b> di pojok kanan bawah.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    <div class="bg-warning text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 40px; height: 45px; flex-shrink: 0; font-weight: bold;">3</div>
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Kerjakan Tugas</h6>
                        <p class="small text-muted mb-0">Selesaikan kuis di akhir bab atau ikuti <b>Try Out</b> untuk menguji kemampuanmu.</p>
                    </div>
                </div>
            </div>
...
<div id="ai-chat-container">
    <div id="ai-chat-window">
        <div class="chat-header">
            <span><i class="fas fa-robot me-2"></i> Asisten PADI</span>
            <button class="btn-close btn-close-white" onclick="toggleChat()"></button>
        </div>
        <div class="chat-body" id="chatBody">
            <div class="msg msg-ai">Halo <strong><?= htmlspecialchars($nama_pengguna) ?></strong>! Ada yang bisa Kak PADI bantu soal pelajaran Bahasa Indonesia hari ini? 😊</div>
            <div id="typingIndicator" class="typing">PADI AI sedang berpikir...</div>
        </div>
        <div class="chat-footer">
            <input type="text" id="chatInput" class="form-control form-control-sm" placeholder="Tanya sesuatu..." onkeypress="handleKeyPress(event)">
...
<script>
   ...
   async function sendMessage() {
    ...
    try {
        ...
        // Cek jika response bukan JSON atau error server
        if (!response.ok) {
        ...
        // Jika API AI mengirimkan pesan error (seperti High Demand)
        if (data.error || !data.jawaban) {
            const errorMsg = data.error || "Kak PADI sedang sibuk membantu siswa lain. Tunggu sebentar lagi ya! 😊";
            appendMessage('ai', "<strong>Maaf:</strong> " + errorMsg);
        } else {
        ...
        typing.style.display = 'none';
        // Pesan jika koneksi terputus atau file PHP tidak ditemukan
        appendMessage('ai', "Waduh, koneksinya terputus. Coba tanya Kak PADI sekali lagi ya? 😊");
