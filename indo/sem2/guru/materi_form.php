...
// =======================================================================
// 1. Handle AJAX Request (Proses Rakit AI Bahasa Indonesia)
// =======================================================================
if (isset($_POST['minta_ai'])) {
    ...
    $prompt = "Tugasmu adalah merakit materi BAHASA INDONESIA menjadi HTML slide interaktif.
               ...
               KETENTUAN KHUSUS B. INDO:
               1. Sajikan teks bacaan atau materi sastra dengan tipografi yang nyaman dibaca.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['minta_ai'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['pesan_sukses'] = "Materi Bahasa Indonesia berhasil disimpan!";
...
<head>
    <meta charset="UTF-8">
    <title>Penyusun Materi B. Indonesia | LITERASI</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <style>
        body { background: #fffcf9; font-family: 'Poppins', sans-serif; }
        .card-custom { border-radius: 15px; box-shadow: 0 6px 15px rgba(0,0,0,0.1); border: none; }
        /* Tema Warna Oranye untuk Bahasa Indonesia */
        .ai-zone { background: #fff3e0; border: 2px dashed #fb8c00; border-radius: 15px; padding: 20px; }
        .preview-window { width: 100%; height: 550px; border: 2px solid #ddd; border-radius: 15px; background: white; }
        .btn-primary { background-color: #fb8c00; border-color: #fb8c00; }
        .btn-primary:hover { background-color: #ef6c00; border-color: #ef6c00; }
...
<nav class="navbar navbar-dark bg-dark py-3 shadow-sm">
    <div class="container">
        <a class="navbar-brand fw-bold" href="dashboard.php"><i class="fas fa-book me-2"></i> B. INDONESIA</a>
        ...
        <div class="row g-4">
            <div class="col-md-7">
                <div class="card card-custom p-4">
                    <h5 class="fw-bold text-primary mb-3"><i class="fas fa-feather-alt"></i> Detail Materi</h5>
                    
                    <div class="mb-3">
                        <label class="fw-bold small text-muted">Judul Materi</label>
                        <input type="text" class="form-control" name="judul" id="judul_form" value="<?= htmlspecialchars($materi['judul']) ?>" placeholder="Contoh: Mengenal Pantun Jenaka" required>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="file_path" value="<?= htmlspecialchars($materi['file_path']) ?>" placeholder="pantun.html">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="fw-bold small text-primary">Isi Konten HTML (Hasil AI / Manual)</label>
                    ...
                    <button type="submit" class="btn btn-primary w-100 fw-bold py-3 rounded-pill shadow">SIMPAN MATERI B. INDONESIA</button>
                </div>
            </div>

            <div class="col-md-5">
                <div class="ai-zone shadow-sm">
                    <h5 class="fw-bold text-primary mb-3"><i class="fas fa-magic"></i> Asisten AI Literasi</h5>
                    <p class="small text-muted">Tuliskan ringkasan materi atau teks bacaan di sini, biar AI yang merakit slidenya.</p>
                    <textarea class="form-control mb-3" id="ai_mentah" rows="14" placeholder="Masukkan materi Bahasa Indonesia..."></textarea>
                    <button type="button" class="btn btn-primary w-100 fw-bold py-3 rounded-pill shadow-sm" id="btnAI">RAKIT SLIDE OTOMATIS</button>
                </div>
            </div>
        </div>
...
<script>
    function updatePreview() {
        ...
        previewDoc.open();
        previewDoc.write(kode || '<body style="color:#ccc;text-align:center;padding-top:100px;font-family:sans-serif;">Pratinjau Masih Kosong</body>');
        previewDoc.close();
    }
    ...
    document.getElementById('btnAI').onclick = function() {
        ...
        if(!judul || !mentah) return Swal.fire('Data Kurang', 'Lengkapi judul dan materi dulu Pak.', 'info');

        this.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Merakit Materi...';
        this.disabled = true;
        ...
        .then(d => {
            ...
            this.disabled = false;
            if(d === "ERROR_TIMEOUT") {
                Swal.fire('Error', 'AI sedang sibuk, coba sesaat lagi.', 'error');
            } else {
                document.getElementById('kode_html').value = d;
                updatePreview();
                Swal.fire('Berhasil!', 'Materi Bahasa Indonesia sudah dirakit.', 'success');
            }
        });
    };
</script>
