...
// Pengecekan Otorisasi Guru dan Admin
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && ($action === 'add' || $action === 'edit')) {
    ...
    if ($action === 'add') {
        ...
        if ($stmt = $db_mapel->prepare($query)) {
            ...
            if ($stmt->execute()) {
                $_SESSION['guru_message'] = "<div class='alert alert-success'>✅ Soal baru berhasil ditambahkan!</div>";
            } else {
                $_SESSION['guru_message'] = "<div class='alert alert-danger'>❌ Gagal menambah soal. Error: " . $stmt->error . "</div>";
            }
            $stmt->close();
        }
        ...
        if ($stmt = $db_mapel->prepare($query)) {
            ...
            if ($stmt->execute()) {
                $_SESSION['guru_message'] = "<div class='alert alert-success'>✅ Soal ID $id berhasil diperbarui!</div>";
            } else {
                $_SESSION['guru_message'] = "<div class='alert alert-danger'>❌ Gagal memperbarui soal. Error: " . $stmt->error . "</div>";
            }
            $stmt->close();
        }
    }
    // Redirect ke halaman kelola_soal.php di folder guru
...
if ($action === 'delete' && $id > 0) {
    ...
    if ($stmt = $db_mapel->prepare($query)) {
        ...
        if ($stmt->execute()) {
            $_SESSION['guru_message'] = "<div class='alert alert-success'>✅ Soal ID $id berhasil dihapus.</div>";
        } else {
            $_SESSION['guru_message'] = "<div class='alert alert-danger'>❌ Gagal menghapus soal.</div>";
        }
        $stmt->close();
    }
    header("Location: kelola_soal.php");
    ...
    if ($stmt_fetch = $db_mapel->prepare($query_fetch)) {
        ...
        if ($result_fetch->num_rows === 1) {
            $soal_data = $result_fetch->fetch_assoc();
             ...
             $_SESSION['guru_message'] = "<div class='alert alert-danger'>Soal tidak ditemukan.</div>";
             header("Location: kelola_soal.php");
             exit();
        }
        $stmt_fetch->close();
...
// Ambil data materi untuk dropdown di form
$materi_list = [];
...
<head>
    ...
    <title>Kelola Soal Quiz | Guru</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>

<div class="container mt-5">
    <h1>Kelola Soal Quiz 🧠 (Akses Guru)</h1>
    ...
    <?php if ($action === 'add' || $action === 'edit'): ?>
        ...
        <div class="card p-4 mb-4">
            ...
            <form method="POST" action="kelola_soal.php?action=<?php echo $action; ?><?php echo ($action === 'edit' ? '&id=' . $id : ''); ?>">
    ...
    ?>
        ...
        <p><a href="kelola_soal.php?action=add" class="btn btn-success">➕ Tambah Soal Baru</a></p>
                ...
                <tbody>
                    <?php if ($result_read && $result_read->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="kelola_soal.php?action=edit&id=<?php echo $soal['id']; ?>" class="btn btn-warning btn-sm me-2">Edit</a>
                                <a href="kelola_soal.php?action=delete&id=<?php echo $soal['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('YAKIN hapus soal ID <?php echo $soal['id']; ?>?')">Hapus</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
