...
$filter_options = [];
$q_materi = $db_mapel->query("SELECT id, judul FROM materi WHERE id_guru = $id_pemilik_ruang ORDER BY judul ASC");
    ...
    while ($row_s = $result_siswa->fetch_assoc()) {
        ...
        if (!empty($filter_materi)) {
            ...
            if (strpos($filter_materi, 'kuis_') === 0) {
                ...
                $res_n = $db_mapel->query("SELECT ROUND(AVG(persentase)) as n FROM riwayat_kuis WHERE id_user = $user_id AND id_materi = $real_id")->fetch_assoc();
            } else {
                $real_id = str_replace('to_', '', $filter_materi);
                $res_n = $db_mapel->query("SELECT ROUND(AVG(persentase)) as n FROM riwayat_tryout WHERE id_user = $user_id AND tryout_id = $real_id")->fetch_assoc();
            }
            ...
            // 1. Kuis per Materi
            $q_k = $db_mapel->query("SELECT ROUND(AVG(persentase)) as n_materi FROM riwayat_kuis WHERE id_user = $user_id GROUP BY id_materi");
            $sum_k = 0; $cnt_k = 0;
            while($rk = $q_k->fetch_assoc()){ $sum_k += $rk['n_materi']; $cnt_k++; }
            $avg_kuis = ($cnt_k > 0) ? round($sum_k / $cnt_k) : null;

            // 2. Tryout per Judul
            $q_t = $db_mapel->query("SELECT ROUND(AVG(persentase)) as n_to FROM riwayat_tryout WHERE id_user = $user_id GROUP BY tryout_id");
            $sum_t = 0; $cnt_t = 0;
        ...
        $res_p = $db_mapel->query("SELECT 
            (SELECT COUNT(DISTINCT id_materi) FROM riwayat_kuis WHERE id_user = $user_id) as m_done,
            (SELECT COUNT(DISTINCT tryout_id) FROM riwayat_tryout WHERE id_user = $user_id) as t_done
        ")->fetch_assoc();
