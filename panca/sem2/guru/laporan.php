...
$filter_options = [];
// MENGGUNAKAN panca_materi
$q_materi = $db_mapel->query("SELECT id, judul FROM " . tbl('panca_materi') . " WHERE id_guru = $id_pemilik_ruang ORDER BY judul ASC");
...
// MENGGUNAKAN panca_tryout_master
$q_tryout = $db_mapel->query("SELECT id, judul FROM " . tbl('panca_tryout_master') . " WHERE id_guru = $id_pemilik_ruang ORDER BY judul ASC");
...
while ($s = $res_siswa->fetch_assoc()) {
    ...
    if (!empty($filter_materi)) {
        if (strpos($filter_materi, 'kuis_') === 0) {
            ...
            $q_n = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) FROM " . tbl('panca_riwayat_kuis') . " WHERE id_user = ? AND id_materi = ?");
        } else {
            $real_id = str_replace('to_', '', $filter_materi);
            $q_n = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) FROM " . tbl('panca_riwayat_tryout') . " WHERE id_user = ? AND tryout_id = ?");
        }
        ...
        // --- LOGIKA PEMBULATAN BERJENJANG (PANCA) ---
        
        // 1. Rata-rata Kuis per Materi
        $q_k = $db_mapel->prepare("SELECT ROUND(AVG(rk.persentase)) as n_materi 
                                   FROM " . tbl('panca_riwayat_kuis'). " rk 
                                   JOIN " . tbl('panca_materi') . " m ON rk.id_materi = m.id 
        ...
        // 2. Rata-rata Tryout per ID
        $q_t = $db_mapel->prepare("SELECT ROUND(AVG(rt.persentase)) as n_to 
                                   FROM " . tbl('panca_riwayat_tryout') . " rt 
                                   JOIN " . tbl('panca_tryout_master') . " tm ON rt.tryout_id = tm.id 
    ...
    // Hitung Progres dengan prefix panca_
    $q_p1 = $db_mapel->prepare("SELECT COUNT(DISTINCT rk.id_materi) FROM " . tbl('panca_riwayat_kuis') . " rk JOIN " . tbl('panca_materi') . " m ON rk.id_materi = m.id WHERE rk.id_user = ? AND m.id_guru = ?");
    $q_p1->bind_param("ii", $user_id, $id_pemilik_ruang);
    $q_p1->execute(); $q_p1->bind_result($ml); $q_p1->fetch(); $q_p1->close();

    $q_p2 = $db_mapel->prepare("SELECT COUNT(DISTINCT rt.tryout_id) FROM " . tbl('panca_riwayat_tryout') . " rt JOIN " . tbl('panca_tryout_master') . " tm ON rt.tryout_id = tm.id WHERE rt.id_user = ? AND tm.id_guru = ?");
...
<div class="container main-card">
    <div class="print-only text-center mb-4">
        ...
        <p style="margin: 0;">Mata Pelajaran: Pend. Pancasila | Semester: Genap 2025/2026</p>
