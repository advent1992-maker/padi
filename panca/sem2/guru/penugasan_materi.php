...
// AMBIL DATA MATERI
// ======================================================================
$stmt = $db_mapel->prepare("SELECT judul, level_kategori FROM panca_materi WHERE id=?");
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'delete') {
        ...
        $res_sid = $db_mapel->query("SELECT ditugaskan_ke FROM panca_penugasan_materi WHERE id=$pid");
        $data_sid = $res_sid->fetch_assoc();

        if ($data_sid) {
            $sid = $data_sid['ditugaskan_ke'];
            // RESET NILAI: Hapus riwayat kuis siswa ini khusus untuk materi ini
            $db_mapel->query("DELETE FROM panca_riwayat_kuis WHERE id_materi=$materi_id AND id_user=$sid");
            // Hapus Penugasan
    ...
    if ($_GET['action'] === 'delete_all_materi') {
        ...
        $db_mapel->query("DELETE FROM panca_riwayat_kuis WHERE id_materi=$materi_id");
        // Hapus Semua Penugasan
        $db_mapel->query("DELETE FROM panca_penugasan_materi WHERE id_materi=$materi_id AND id_guru=$id_guru");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_penugasan'])) {
    ...
    $stmt = $db_mapel->prepare("
        INSERT INTO panca_penugasan_materi
