...
if (isset($_GET['toggle_id'])) {
    ...
    $stmt = $db_mapel->prepare("UPDATE " . tbl('panca_tryout_master') . " SET tampilkan = ? WHERE id = ? AND id_guru = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adopsi_tryout'])) {
    ...
    $stmt_o = $db_mapel->prepare("SELECT * FROM " . tbl('panca_tryout_master') . " WHERE id = ?");
    ...
    if ($data_orig) {
        ...
        $stmt_i = $db_mapel->prepare("INSERT INTO " . tbl('panca_tryout_master') . " (judul, jenis_ujian, kelas, waktu_alokasi, id_guru) VALUES (?, ?, ?, ?, ?)");
        ...
        if ($stmt_i->execute()) {
            ...
            $q_soal = "INSERT INTO " . tbl('panca_soal_tryout') . " (tryout_id, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, jawaban_benar, gambar_url)
                       SELECT ?, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, jawaban_benar, gambar_url
                       FROM " . tbl('panca_soal_tryout') . " WHERE tryout_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_tryout'])) {
    ...
    if (!empty($judul) && !empty($jenis_ujian) && !empty($kelas) && is_numeric($waktu_alokasi) && $waktu_alokasi > 0) {
        $query = "INSERT INTO " . tbl('panca_tryout_master') . " (judul, jenis_ujian, kelas, waktu_alokasi, id_guru) VALUES (?, ?, ?, ?, ?)";
        $stmt = $db_mapel->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_tryout'])) {
    ...
    $query = "UPDATE " . tbl('panca_tryout_master') . " SET judul=?, jenis_ujian=?, kelas=?, waktu_alokasi=? WHERE id=? AND id_guru=?";
    $stmt = $db_mapel->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_tryout'])) {
    ...
    $db_mapel->query("DELETE FROM " . tbl('panca_soal_tryout') . " WHERE tryout_id = $tryout_id");
    $db_mapel->query("DELETE FROM " . tbl('panca_riwayat_tryout') . " WHERE tryout_id = $tryout_id");

    $query = "DELETE FROM " . tbl('panca_tryout_master') . " WHERE id = ? AND id_guru = ?";
    $stmt = $db_mapel->prepare($query);
...
$query_pribadi = "SELECT tm.*, COUNT(st.id) AS total_soal FROM " . tbl('panca_tryout_master') . " tm LEFT JOIN " . tbl('panca_soal_tryout') . " st ON tm.id = st.tryout_id WHERE tm.kelas IN ($placeholders) AND tm.id_guru = ? GROUP BY tm.id ORDER BY tm.id DESC";
$tryout_pribadi = execute_query($db_mapel, $query_pribadi, array_merge($kelas_array, [$user_id]), str_repeat("s", count($kelas_array))."i");

// Query untuk Mapel Apapun (IPAS/MTK/B.Inggris)
$query_adopsi = "SELECT tm.*, COUNT(st.id) AS total_soal 
                 FROM " . tbl('panca_tryout_master') . " tm 
                 LEFT JOIN " . tbl('panca_soal_tryout') . " st ON tm.id = st.tryout_id 
                 WHERE tm.kelas IN ($placeholders) 
                 AND tm.id_guru != ? 
                 AND tm.judul NOT IN (SELECT judul FROM " . tbl('panca_tryout_master') . " WHERE id_guru = ?) 
                 GROUP BY tm.judul, tm.kelas 
                 ORDER BY tm.id DESC";
                ...
                <table class="table table-hover align-middle table-striped mb-0 table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><span class="badge bg-info text-dark fs-6"><?php echo htmlspecialchars($tryout['total_soal']); ?></span></td>
                            <td class="text-center">
                                <!--<a href="form_soal_tryout.php?tryout_id=<?php echo $tryout['id']; ?>&mode=view" class="btn btn-sm btn-outline-primary me-2 mb-1"><i class="fas fa-eye"></i> Lihat</a>-->
                                ...
                                <!--<a href="penugasan_tryout.php?tryout_id=<?php echo $tryout['id']; ?>" class="btn btn-sm btn-warning fw-bold mb-1"><i class="fas fa-calendar-check"></i> Tugaskan</a>-->
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
