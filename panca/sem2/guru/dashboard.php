...
// Penentuan nama tabel dinamis (mendukung Mode Arsip & Aktif)
$tbl_materi = tbl('panca_materi');
$tbl_soal   = tbl('panca_soal');

// Statistik Materi
$res_materi = $db_mapel->query("SELECT COUNT(id) AS total FROM {$tbl_materi} $guru_filter");
$stats['total_materi'] = $res_materi ? ($res_materi->fetch_assoc()['total'] ?? 0) : 0;

// Statistik Kuis
$res_kuis = $db_mapel->query("SELECT COUNT(DISTINCT t1.materi_id) AS total FROM {$tbl_soal} t1 JOIN {$tbl_materi} t2 ON t1.materi_id = t2.id $guru_filter_join");
$stats['total_kuis_dibuat'] = $res_kuis ? ($res_kuis->fetch_assoc()['total'] ?? 0) : 0;

// Statistik Guru
$res_guru = $conn->query("SELECT COUNT(id) AS total FROM users WHERE role = 'guru'");
$stats['total_guru'] = $res_guru ? ($res_guru->fetch_assoc()['total'] ?? 0) : 0;
...
<head>
    ...
    <title>Dashboard <?php echo ucfirst($role); ?> | <?= strtoupper($nama_mapel ?? 'PANCASILA') ?></title>
...
<nav class="navbar navbar-expand-lg navbar-dark navbar-custom py-3 shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="dashboard.php">
            <strong><?= strtoupper($nama_mapel ?? 'PANCASILA') ?> | <?php echo strtoupper($role); ?></strong>
        </a>
...
<?php
if (isset($db_mapel)) { $db_mapel->close(); }
