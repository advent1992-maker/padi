...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Ambil judul materi
    $q = $db_mapel->query("SELECT judul FROM panca_materi WHERE id = $id_materi");
    $m = $q->fetch_assoc();
    $topik = $m['judul'];

    // RAKIT PROMPT DENGAN TINGKAT KESULITAN
    $prompt = "Kamu adalah pakar pembuat soal pendidikan pancasila. Buatkan $jumlah soal pilihan ganda tentang '$topik' untuk siswa kelas 5 SD.
    ...
    if ($data_soal) {
        foreach ($data_soal as $s) {
            ...
            $db_mapel->query("INSERT INTO panca_soal (materi_id, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, jawaban_benar)
    ...
    <body>
        <script>
            Swal.fire({
                title: 'Berhasil!',
                text: '$jumlah soal tingkat $kesulitan telah ditambahkan ke materi.',
                icon: 'success',
