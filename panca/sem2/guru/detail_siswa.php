...
$stmt_to_stats = $db_mapel->prepare("SELECT AVG(persentase) AS avg_to, COUNT(DISTINCT tryout_id) AS total_to FROM panca_riwayat_tryout WHERE id_user = ?");
...
<div class="container pb-5">
    ...
    <div class="table-responsive bg-white rounded shadow-sm">
        <table class="table table-hover align-middle mb-0" id="kuis-table">
            ...
            <tbody>
                <?php
                $stmt_rk = $db_mapel->prepare("SELECT rk.id, rk.persentase, rk.tanggal_dikerjakan, m.judul FROM panca_riwayat_kuis rk JOIN panca_materi m ON rk.id_materi = m.id WHERE rk.id_user = ? ORDER BY rk.tanggal_dikerjakan DESC");
