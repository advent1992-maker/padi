...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Ambil info tryout
    $q = $db_mapel->query("SELECT judul, kelas FROM panca_tryout_master WHERE id = $tryout_id");
    $tm = $q->fetch_assoc();
    $judul_ujian = $tm['judul'];
    $kelas = $tm['kelas'];

    $prompt = "Kamu adalah spesialis pembuat soal ujian pendidikan pancasila. Buatkan $jumlah soal pilihan ganda untuk ujian '$judul_ujian' kelas $kelas SD.
    ...
    if ($data_soal) {
        foreach ($data_soal as $s) {
            ...
            $db_mapel->query("INSERT INTO panca_soal_tryout (tryout_id, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, jawaban_benar)
