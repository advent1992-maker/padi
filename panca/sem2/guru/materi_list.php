...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        $db_mapel->query("DELETE FROM " . tbl('panca_riwayat_kuis') . " WHERE id_materi = $delete_id");

        // 2. Hapus Soal
        $db_mapel->query("DELETE FROM " . tbl('panca_soal') . " WHERE materi_id = $delete_id");

        // 3. Hapus Materi Utama
        $stmt_del = $db_mapel->prepare("DELETE FROM " . tbl('panca_materi') . " WHERE id = ?");
...
$query_pribadi = "SELECT m.*, COUNT(s.id) AS jumlah_soal FROM " . tbl('panca_materi') . " m
...
$query_adopsi = "SELECT m.*, COUNT(s.id) AS jumlah_soal FROM " . tbl('panca_materi') . " m
                 LEFT JOIN " . tbl('panca_soal') . " s ON m.id = s.materi_id
...
<head>
    <meta charset="UTF-8">
    <title>Kelola Materi | PANCASILA</title>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
    <!--<a href="penugasan_materi.php?id_materi=<?= $a['id'] ?>&mode=adopsi" class="btn btn-sm btn-primary">-->
    <!--    <i class="fas fa-bullhorn"></i> Tugaskan-->
    <!--</a>-->
