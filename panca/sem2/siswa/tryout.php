...
";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['soal_id'])) {
       ...
       $query_save = "
    INSERT INTO panca_tryout_jawaban (session_id, soal_id, jawaban_siswa)
    VALUES (?, ?, ?)
...
// --- B. LOGIKA START TRY OUT (Mode Konfirmasi) ---
    ...
    FROM panca_tryout_master tm
    LEFT JOIN panca_soal_tryout st ON tm.id = st.tryout_id
...
";
    ...
    if ($session_check) {
        if (!empty($session_check['waktu_selesai'])) {
            ...
            $query_upaya = "SELECT COUNT(id) AS total_upaya FROM panca_riwayat_tryout WHERE tryout_id = ? AND id_user = ?";
    ...
    if (empty($errors) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['start_tryout'])) {
        ...
        $query_insert = "
    INSERT INTO panca_tryout_session (tryout_id, siswa_id, waktu_mulai, waktu_selesai_target, status)
    VALUES (?, ?, ?, ?, 'ongoing')
";
