...
// === Ambil Data Materi ===
$query_materi = "SELECT judul, level_kategori FROM panca_materi WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query_kunci = "SELECT id, jawaban_benar FROM panca_soal WHERE id IN ($placeholders)";
    ...
    // Simpan Riwayat
    $query_action = "INSERT INTO panca_riwayat_kuis (id_user, id_materi, skor, total_soal, persentase, status_lulus, tanggal_dikerjakan) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    // Simpan Detail
    $query_insert_detail = "INSERT INTO panca_hasil_quiz (user_id, riwayat_kuis_id, soal_id, jawaban_siswa, skor, waktu_jawab) VALUES (?, ?, ?, ?, ?, ?)";
...
$query_soal = "SELECT id, pertanyaan, opsi_a, opsi_b, opsi_c, opsi_d, 
               ...
               gambar_url FROM panca_soal WHERE materi_id = ? ORDER BY RAND() LIMIT 15";
...
<head>
    ...
    <title><?= htmlspecialchars($page_title); ?> | Pendidikan Pancasila</title>
...
<div class="container mt-5">
    ...
    <form method="POST" action="start_quiz.php?materi_id=<?= $id_materi; ?>">
        <?php foreach ($soal_list as $index => $soal): ?>
            <div class="quiz-card">
                <h5>Soal <?= $index + 1; ?>.</h5>
                <div class="fs-5 tex2jax_process mb-2"><?= $soal['pertanyaan']; ?></div>
