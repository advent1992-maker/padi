...
if ($id_guru_siswa > 0) {
    // 1. STATISTIK KUIS PANCASILA (Gunakan panca_riwayat_kuis)
    $list_nilai_kuis = [];
    $q_kuis_per_mapel = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) as nilai_bulat_materi FROM panca_riwayat_kuis WHERE id_user = ? GROUP BY id_materi");
    ...
    // 2. STATISTIK TRYOUT PANCASILA (Gunakan panca_riwayat_tryout)
    $list_nilai_to = [];
    $q_to_per_judul = $db_mapel->prepare("SELECT ROUND(AVG(persentase)) as nilai_bulat_to FROM panca_riwayat_tryout WHERE id_user = ? GROUP BY tryout_id");
    ...
    $total_avg = 0;
    $count_valid = 0;
    if ($avg_kuis_final > 0) { $total_avg += $avg_kuis_final; $count_valid++; }
    if ($avg_to_final > 0) { $total_avg += $avg_to_final; $count_valid++; }

    $rata_rata_gabungan = ($count_valid > 0) ? round($total_avg / $count_valid) : 0;

    // 4. UPDATE STATISTIK BOX (Gunakan panca_...)
    $q_stat = $db_mapel->prepare("SELECT 
        ...
        FROM panca_riwayat_kuis WHERE id_user = ?");
    ...
    $q_to_count = $db_mapel->prepare("SELECT COUNT(DISTINCT tryout_id) as total_to FROM panca_riwayat_tryout WHERE id_user = ?");
...
<head>
    ...
    <title>Dashboard Siswa | PEND. PANCASILA</title>
    ...
    /* Warna Merah Terang untuk Pendidikan Pancasila */
    ...
    .hero-card {
        background: <?= $is_archive ? 'linear-gradient(135deg, #6c757d, #495057)' : 'linear-gradient(135deg, #dc3545, #b02a37)' ?>;
    ...
    /* Style Petunjuk Pancasila */
    .guide-banner-pancasila { background: #fff; border: none; border-radius: 15px; border-left: 6px solid #dc3545; }
...
<nav class="navbar navbar-dark <?= $is_archive ? 'bg-secondary' : 'bg-primary' ?> shadow-sm">
    <div class="container">
        <a class="navbar-brand fw-bold" href="#">PEND. PANCASILA | SISWA</a>
...
<div class="container mt-5 mb-5 text-start">
    <div class="hero-card text-center mb-5">
        <h1 class="display-5 fw-bold"><i class="fas fa-flag"></i> Peta Pembelajaran Anda</h1>
        <p class="lead mt-3">Selamat datang kembali, <?php echo htmlspecialchars($nama_pengguna); ?>! Mari selesaikan materi Pendidikan Pancasila hari ini bersama <b>Guru <?php echo htmlspecialchars($nama_guru_pembimbing); ?></b>.</p>
    ...
    <div class="row mb-5">
        <div class="col-12">
            <div class="card guide-banner-pancasila shadow-sm">
                <div class="card-body p-4 d-flex align-items-center justify-content-between flex-wrap">
                    <div>
                        <h5 class="fw-bold mb-1 text-primary"><i class="fas fa-info-circle me-2"></i> Petunjuk Belajar Pancasila</h5>
                        <p class="text-muted mb-0 small">Pelajari alur materi, kuis, dan penggunaan dashboard serta Asisten AI di sini.</p>
                    </div>
                    <button class="btn btn-primary fw-bold px-4 rounded-pill mt-3 mt-md-0 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalPanduanPancasila">
    ...
    <div class="row g-4">
        <div class="col-md-6 mb-3">
            <div class="stat-card p-4 h-100">
                ...
                <p>Pilih materi Pendidikan Pancasila yang ingin Anda taklukkan hari ini.</p>
                <a href="materi.php" class="btn btn-primary w-100 btn-lg">
                    <i class="fas fa-landmark"></i> Lihat Materi
                </a>
            </div>
        </div>
        <div class="col-md-6 mb-3">
            <div class="stat-card p-4 h-100">
                ...
                <a href="riwayat_progress.php" class="btn btn-success w-100 btn-lg" style="background-color: #28a745; border-color: #28a745;">
        ...
        <div class="col-md-6 mb-3">
            <div class="stat-card p-4 h-100">
                <h4><i class="fas fa-file-signature"></i> Mulai Try Out</h4>
...
<div class="modal fade" id="modalPanduanPancasila" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px; border: none;">
            <div class="modal-header border-0 p-4 pb-0 text-start">
                <h5 class="fw-bold text-primary"><i class="fas fa-flag me-2"></i> Alur Belajar Pendidikan Pancasila</h5>
            ...
            <div class="modal-body p-4 text-start">
                <div class="d-flex mb-4">
                    ...
                    <div class="ms-3">
                        ...
                        <p class="small text-muted mb-0">Buka menu <b>Lihat Materi</b>, pelajari isinya, dan kerjakan kuis di akhir bab.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    ...
                    <div class="ms-3">
                        ...
                        <p class="small text-muted mb-0">Jika ada materi yang kurang dipahami, tanya <b>Kak PADI AI</b> di pojok kanan bawah.</p>
                    </div>
                </div>
                <div class="d-flex mb-4">
                    ...
                    <div class="ms-3">
                        <h6 class="fw-bold mb-1">Evaluasi & Peringkat</h6>
                        <p class="small text-muted mb-0">Ikuti <b>Tryout</b> untuk menguji pemahaman akhirmu dan lihat peringkatmu di <b>Leaderboard</b>.</p>
                    </div>
                </div>
            </div>
...
<div id="ai-chat-container">
    <div id="ai-chat-window">
        <div class="chat-header">
            <span><i class="fas fa-robot me-2"></i> Asisten PADI Pendidikan Pancasila</span>
            <button class="btn-close btn-close-white" onclick="toggleChat()"></button>
        </div>
        <div class="chat-body" id="chatBody">
            <div class="msg msg-ai">Halo <strong><?= htmlspecialchars($nama_pengguna) ?></strong>! Ada materi Pendidikan Pancasila yang ingin ditanyakan? Kak Padi bantu ya! 😊</div>
            <div id="typingIndicator" class="typing">PADI AI sedang berpikir...</div>
        </div>
        <div class="chat-footer">
            <input type="text" id="chatInput" class="form-control form-control-sm" placeholder="Tanya tentang Pancasila..." onkeypress="handleKeyPress(event)">
...
<script>
    ...
    try {
        const response = await fetch('proses_ai_asisten.php', {
            ...
            body: JSON.stringify({ 
                pesan: pesan,
                mapel: 'Pancasila' // TAMBAHKAN BARIS INI AGAR AI TAHU LOKASINYA
            })
        });
