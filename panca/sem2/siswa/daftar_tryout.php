<?php
// FILE: siswa/daftar_tryout.php (Revisi Modul Pancasila)
...
$query = "
    SELECT 
        ...
        tm.jenis_ujian, 
        tm.waktu_alokasi,
        (SELECT COUNT(rt.id) FROM panca_riwayat_tryout rt WHERE rt.tryout_id = tm.id AND rt.id_user = ?) AS riwayat_count,
        (SELECT AVG(rt.skor) FROM panca_riwayat_tryout rt WHERE rt.tryout_id = tm.id AND rt.id_user = ?) AS avg_skor,
        (SELECT AVG(rt.persentase) FROM panca_riwayat_tryout rt WHERE rt.tryout_id = tm.id AND rt.id_user = ?) AS avg_persentase
    FROM 
        panca_tryout_master tm
    WHERE 
        tm.kelas = ? 
        AND tm.id_guru = ?
        AND tm.tampilkan = 1
...
<head>
    ...
    <title>Daftar Try Out | PEND. PANCASILA</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <style>
        body { background-color: #f0f8ff; }
        .header-blue { background-color: #dc3545; color: white; padding: 30px 0; }
