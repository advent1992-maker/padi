...
$stmt = $db_mapel->prepare("SELECT id, username, nama_lengkap, email, kelas FROM users WHERE id = ? AND role = 'siswa'");
if (!$stmt) {
    die("Error menyiapkan query data pengguna: " . $db_mapel->error);
}
...
if (!$stmt_kuis) {
    die("Error menyiapkan query riwayat kuis: " . $db_mapel->error);
}
...
";
$stmt_tryout = $db_mapel->prepare($tryout_query);
if (!$stmt_tryout) {
    die("Error menyiapkan query riwayat tryout: " . $db_mapel->error);
}
