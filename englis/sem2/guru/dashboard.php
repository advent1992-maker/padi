...
// Cek apakah sedang dalam mode kolaborasi (mengelola kelas orang lain)
if (isset($_GET['id_guru_target'])) {
    $user_id = (int)$_GET['id_guru_target'];
    $_SESSION['id_guru_pilihan'] = $user_id; // Simpan di session agar menu lain (materi_list, dll) ikut berubah
} else {
    // Jika tidak ada di URL, gunakan session pilihan, jika tidak ada juga baru pakai ID sendiri
    $user_id = $_SESSION['id_guru_pilihan'] ?? (int)($_SESSION['user_id'] ?? $_SESSION['id'] ?? 0);
}

// Info Tambahan: Mengetahui apakah sedang mengelola kelas sendiri atau orang lain
$is_kolaborasi = (isset($_SESSION['id_guru_pilihan']) && $_SESSION['id_guru_pilihan'] != ($_SESSION['user_id'] ?? $_SESSION['id']));
...
// 5. AMBIL DAFTAR SISWA (DATABASE PUSAT)
$daftar_siswa = [];
...
<head>
    ...
    <title>Dashboard <?php echo ucfirst($role); ?> | B.INGGRIS</title>
...
<nav class="navbar navbar-expand-lg navbar-dark navbar-custom py-3 shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="dashboard.php">
            <strong>B.INGGRIS | <?php echo strtoupper($role); ?></strong>
        </a>
...
<?php if ($is_kolaborasi): ?>
    <div class="alert alert-info border-0 shadow-sm rounded-4 d-flex justify-content-between align-items-center mb-4">
        <span><i class="fas fa-info-circle me-2"></i> Anda sedang mengelola kelas <strong>Guru Lain</strong>.</span>
...
<?php endif; ?>
    ...
    <div class="row g-4">
        ...
        <!--<div class="col-md-4">-->
        <!--    <div class="card card-menu bg-warning h-100 shadow">-->
        <!--        <div class="card-body p-4 text-center text-dark">-->
        <!--            <i class="fas fa-star-half-alt icon-large"></i>-->
        <!--            <h4 class="fw-bold">Uji Kelayakan</h4>-->
        <!--            <p class="small opacity-75">Penilaian kelayakan media pembelajaran.</p>-->
        <!--            <a href="form_uji_guru.php" class="btn btn-dark text-white fw-bold w-100 mt-3 rounded-pill">Mulai</a>-->
        <!--        </div>-->
        <!--    </div>-->
        <!--</div>-->
    </div>
