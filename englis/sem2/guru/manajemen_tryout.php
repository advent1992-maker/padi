...
if (isset($_GET['toggle_id'])) {
    ...
    $stmt = $db_mapel->prepare("UPDATE tryout_master SET tampilkan = ? WHERE id = ? AND id_guru = ?");
...
$query_pribadi = "SELECT tm.*, COUNT(st.id) AS total_soal FROM " . tbl('tryout_master') . " tm LEFT JOIN soal_tryout st ON tm.id = st.tryout_id WHERE tm.kelas IN ($placeholders) AND tm.id_guru = ? GROUP BY tm.id ORDER BY tm.id DESC";
