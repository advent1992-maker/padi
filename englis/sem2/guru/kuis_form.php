<?php
// FILE: guru/kuis_form.php (ENGLISH) - Disinkronkan dengan Fitur IPAS (Pembahasan + Upload)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_owner_or_admin) {
    ...
    $pembahasan = $_POST['pembahasan']; // Mengambil data pembahasan
    ...
    if (isset($_POST['update_soal'])) {
        // SQL UPDATE dengan Pembahasan (Total 12 Kolom + 1 ID)
        $sql = "UPDATE soal SET 
        ...
        // SQL INSERT dengan Pembahasan (Total 13 Kolom)
...
<head>
    <meta charset="UTF-8">
    <title>Manage English Quiz</title>
    ...
    <style>
        .opsi-box { border: 1px solid #ddd; padding: 15px; border-radius: 10px; background: #fff; margin-bottom: 10px; transition: 0.3s; }
        .opsi-box:hover { border-color: #0d6efd; }
        .pembahasan-box { border: 1px solid #0dcaf0; background: #f0fbff; padding: 15px; border-radius: 10px; }
        .img-preview { max-height: 120px; border: 1px solid #ccc; margin-top: 5px; border-radius: 5px; }
    </style>
</head>
<body class="bg-light">
...
<nav class="navbar navbar-dark bg-primary p-3 shadow-sm">
    <div class="container">
        <a class="navbar-brand fw-bold" href="#"><i class="fas fa-book-open me-2"></i> ENGLISH TEACHER DASHBOARD</a>
    </div>
</nav>

<div class="container mt-4 mb-5">
    <h3><i class="fas fa-language text-primary"></i> Manage: <?= htmlspecialchars($materi['judul']) ?></h3>
    <a href="kuis_list.php" class="btn btn-outline-secondary mb-4 btn-sm rounded-pill"><i class="fas fa-arrow-left"></i> Back to List</a>
        ...
        <div class="card-body p-4">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4 p-3 bg-light border rounded-3">
                    <label class="fw-bold text-primary">Question Image</label>
                    <input type="text" name="gambar_url" class="form-control mb-2" placeholder="Manual filename (e.g., english1.jpg)" value="<?= $edit_data['gambar_url'] ?? '' ?>">
                    <input type="file" name="file_pertanyaan" class="form-control form-control-sm">
                </div>

                <h5 class="mb-3 fw-bold">Answer Options</h5>
                <div class="row">
                    <?php foreach(['a','b','c','d'] as $k): ?>
                    <div class="col-md-6">
                        <div class="opsi-box">
                            <label class="fw-bold">Option <?= strtoupper($k) ?></label>
                            <input type="text" name="opsi_<?= $k ?>" class="form-control mb-2" value="<?= $edit_data['opsi_'.$k] ?? '' ?>" placeholder="Option text">
                            <input type="text" name="opsi_<?= $k ?>_gambar_url" class="form-control mb-2 form-control-sm" placeholder="Image filename" value="<?= $edit_data['opsi_'.$k.'_gambar_url'] ?? '' ?>">
                ...
                <div class="mb-4">
                    <label class="fw-bold text-danger">Correct Answer</label>
                ...
                <div class="mb-4 pembahasan-box">
                    <label class="fw-bold text-info"><i class="fas fa-lightbulb"></i> Explanation / Pembahasan</label>
                    <textarea name="pembahasan" class="form-control" rows="4" placeholder="Explain why this answer is correct..."><?= $edit_data['pembahasan'] ?? '' ?></textarea>
                    <small class="text-muted italic">This will be shown to students after they finish the quiz.</small>
                </div>

                <button type="submit" name="<?= $edit_id ? 'update_soal' : 'tambah_soal' ?>" class="btn btn-primary w-100 fw-bold py-2 shadow-sm">
                    <i class="fas fa-save me-2"></i> <?= $edit_id ? 'UPDATE QUESTION' : 'SAVE NEW QUESTION' ?>
                </button>
            </form>
        </div>
    </div>
    ...
    <h4 class="mb-3">Question List (Total: <?= $soal_list->num_rows ?>)</h4>
                    ...
                    <div class="flex-grow-1">
                        ...
                        <?php if(!empty($s['pembahasan'])): ?>
                            <div class="mt-3 p-2 bg-light rounded border-start border-info border-3 small">
                                <strong class="text-info">Explanation:</strong><br>
                                <?= nl2br(htmlspecialchars($s['pembahasan'])) ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="ms-3 text-end">
                        <span class="badge bg-primary px-3 py-2 mb-2 shadow-sm">Key: <?= $s['jawaban_benar'] ?></span>
                        <div class="btn-group d-block">
                            <a href="?id_materi=<?= $id_materi ?>&edit_id=<?= $s['id'] ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit text-white"></i></a>
                            <a href="kuis_action.php?action=hapus&id_materi=<?= $id_materi ?>&soal_id=<?= $s['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this question?')"><i class="fas fa-trash"></i></a>
                        </div>
                    </div>
                </div>
            </div>
