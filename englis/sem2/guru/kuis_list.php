<?php
// ======================================================================================
// KUIS_LIST.PHP (SISI GURU) - ENGLISH (Fitur Tampilkan/Sembunyikan Kuis)
// ======================================================================================
...
if (isset($_GET['toggle_id'])) {
    ...
    $stmt = $db_mapel->prepare("UPDATE " . tbl('materi') . " SET tampilkan_kuis = ? WHERE id = ? AND id_guru = ?");
...
// ----------------------------------------------------
// 1. QUERY MATERI PRIBADI (Ditambah kolom tampilkan_kuis)
// ----------------------------------------------------
...
$query_adopsi = "
    SELECT m.id, m.judul, m.level_kategori, COUNT(s.id) AS jumlah_soal
    FROM " . tbl('materi') . " m
    LEFT JOIN soal s ON m.id = s.materi_id
...
<head>
    <meta charset="UTF-8">
    <title>Manage Quiz Questions | ENGLISH</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background-color: #f8faff; font-family: 'Poppins', sans-serif; }
        .card-table { border-radius: 15px; overflow: hidden; border: none; box-shadow: 0 5px 15px rgba(0,0,0,0.08); }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark p-3">
    <div class="container">
        <a class="navbar-brand" href="dashboard.php"><strong>ENGLISH | GURU</strong></a>
...
<div class="container mt-5">
    <header class="mb-4">
        <h1><i class="fas fa-language text-primary"></i> Manage Quiz Questions</h1>
        <p class="text-muted">Active Grade(s): <b><?= htmlspecialchars($kelas_diajar_str) ?></b></p>
    </header>

    <a href="dashboard.php" class="btn btn-outline-secondary mb-4"><i class="fas fa-arrow-left"></i> Dashboard</a>

    <h4 class="text-success mb-3"><i class="fas fa-user-check"></i> Private Material</h4>
    <div class="card card-table mb-5">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-primary" style="background-color: #6f42c1;">
                    <tr>
                        <th class="text-white">#</th>
                        <th class="text-white">Topic Title</th>
                        <th class="text-white">Quiz Status</th>
                        <th class="text-center text-white">Questions</th>
                        <th class="text-center text-white">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($materi_pribadi)): ?>
                        <tr><td colspan="5" class="text-center p-4">You haven't created any English materials yet.</td></tr>
                    <?php else: $no=1; foreach($materi_pribadi as $m): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><b><?= htmlspecialchars($m['judul']) ?></b> <br><small class="text-muted">Grade <?= $m['level_kategori'] ?></small></td>
                            <td>
                                <?php if($m['tampilkan_kuis'] == 1): ?>
                                    <a href="?toggle_id=<?= $m['id'] ?>&status=1" class="btn btn-sm btn-light border text-success fw-bold rounded-pill px-3 shadow-sm btn-toggle">
                                        <i class="fas fa-eye me-1"></i> Visible
                                    </a>
                                <?php else: ?>
                                    <a href="?toggle_id=<?= $m['id'] ?>&status=0" class="btn btn-sm btn-secondary fw-bold rounded-pill px-3 btn-toggle">
                                        <i class="fas fa-eye-slash me-1"></i> Hidden
                                    </a>
                                <?php endif; ?>
                            </td>
                            <td class="text-center"><span class="badge bg-info text-dark"><?= $m['jumlah_soal'] ?> Items</span></td>
                            <td class="text-center">
                                <a href="kuis_form.php?id_materi=<?= $m['id'] ?>" class="btn btn-sm btn-success fw-bold">
                                    <i class="fas fa-edit"></i> Edit Soal Manual
                                </a>
                                <a href="ai_generator_page.php?id_materi=<?= $m['id'] ?>" class="btn btn-sm btn-success fw-bold" onclick="this.innerHTML='<i class=\'fas fa-spinner fa-spin\'></i> Connecting AI...';">
    ...
    <!--<h4 class="text-primary mb-3"><i class="fas fa-star"></i> Other Teacher's Materials</h4>-->
    <!--<div class="card card-table shadow-sm">-->
    <!--    <div class="table-responsive">-->
    <!--        <table class="table table-hover align-middle mb-0">-->
    <!--            <thead class="table-dark">-->
    <!--                <tr><th>#</th><th>Topic Title</th><th>Grade</th><th class="text-center">Action</th></tr>-->
    <!--            </thead>-->
    <!--            <tbody>-->
    <!--                <?php if (empty($materi_adopsi)): ?>-->
    <!--                    <tr><td colspan="4" class="text-center p-4">No other materials available.</td></tr>-->
    <!--                <?php else: $no=1; foreach($materi_adopsi as $a): ?>-->
    <!--                    <tr>-->
    <!--                        <td><?= $no++ ?></td>-->
    <!--                        <td><?= htmlspecialchars($a['judul']) ?></td>-->
    <!--                        <td>Grade <?= $a['level_kategori'] ?></td>-->
    <!--                        <td class="text-center">-->
    <!--                            <a href="kuis_form.php?id_materi=<?= $a['id'] ?>&mode=view" class="btn btn-sm btn-outline-primary">-->
