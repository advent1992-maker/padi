...
if ($materi_id > 0 && $level_kelas > 0) {
    ...
    if ($materi) {
        ...
        if ($has_quiz) {
            ...
            $query_last_result = "SELECT skor, total_soal, persentase, status_lulus, tanggal_dikerjakan FROM riwayat_kuis WHERE id_user = ? AND id_materi = ? ORDER BY tanggal_dikerjakan DESC LIMIT 1";
...
<head>
    ...
    <title><?= $materi ? htmlspecialchars($materi['judul']) : 'Material Detail'; ?> | ENGLISH</title>
    ...
    <style>
        ...
        .result-card { border: 1px solid #dee2e6; border-left: 5px solid #6f42c1; padding: 15px; border-radius: 8px; background-color: #f8f9fa; }
...
<nav class="navbar navbar-expand-lg navbar-dark shadow-sm" style="background-color: #6f42c1;">
    <div class="container">
        <a class="navbar-brand fw-bold" href="dashboard.php"><i class="fas fa-language me-2"></i>ENGLISH CLASS</a>
        <div class="navbar-text text-white d-none d-md-block">
            Hello, <b class="text-warning"><?php echo htmlspecialchars($nama_pengguna); ?></b>
        </div>
        <a href="../logout.php" class="btn btn-sm btn-light fw-bold" style="color:#6f42c1;">Keluar</a>
    </div>
</nav>

<div class="container mt-3 mb-5">
    <a href="materi.php" class="btn btn-sm btn-outline-secondary mb-3"><i class="fas fa-arrow-left"></i> Kembali / Back</a>
                ...
                <script>
                    (function() {
                        ...
                        try {
                            ...
                            const html = b64ToUtf8(base64Data);
            ...
            <?php if ($has_quiz): ?>
                ...
                <?php if ($last_result): ?>
                    <div class="result-card mb-4" style="border-left-color: <?= ($last_result['status_lulus'] == 'LULUS' ? '#198754' : '#dc3545'); ?>;">
                        <h5 class="mb-2">Last Quiz Result:</h5>
                        <p class="mb-1 small">
                            Score: <b><?= $last_result['skor']; ?>/<?= $last_result['total_soal']; ?></b> (<?= round($last_result['persentase']); ?>%) - 
                            <span class="badge bg-<?= ($last_result['status_lulus'] == 'LULUS' ? 'success' : 'danger'); ?>">
                                <?= $last_result['status_lulus']; ?>
                            </span>
                        </p>
                ...
                <?php if ($materi['tampilkan_kuis'] == 1): ?>
                    <div class="alert alert-info text-center py-4 <?= ($jumlah_percobaan >= 5) ? 'limit-reached' : ''; ?>">
                        <h6 class="fw-bold">Have you understood the lesson?</h6>
                        <p class="mb-2">Attempts used: <b><?= $jumlah_percobaan; ?> / 5</b></p>
                        
                        <?php if ($jumlah_percobaan < 5): ?>
                            <p class="small">Let's test your skills with a quiz!</p>
                            <a href="start_quiz.php?materi_id=<?php echo $materi['id']; ?>" class="btn btn-primary px-5 fw-bold shadow-sm rounded-pill" style="background-color:#6f42c1; border-color:#6f42c1;">
                                <i class="fas fa-pencil-alt me-2"></i>
                                <?php echo $last_result ? 'Retake Quiz' : 'Start Quiz'; ?>
                            </a>
                        <?php else: ?>
                            <div class="alert alert-danger d-inline-block rounded-pill px-4 mt-2">
                                <i class="fas fa-exclamation-triangle me-2"></i> Sorry, you have used all 5 attempts.
                            </div>
                        <?php endif; ?>
                    </div>
                <?php else: ?>
                    <div class="alert alert-warning text-center border-warning py-3 rounded-pill shadow-sm">
                        <i class="fas fa-lock me-2"></i> <b>The quiz is currently disabled by your teacher.</b>
                    </div>
                <?php endif; ?>

            <?php else: ?>
                <div class="alert alert-light text-center border">Quiz is not available for this lesson.</div>
            <?php endif; ?>
        </div>
    <?php endif; ?>
</div>
